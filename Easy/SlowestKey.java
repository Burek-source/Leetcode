...
class Solution 
{
    ...
    {
        char longestKeyPressed = keysPressed.charAt(0);
        int val = releaseTimes[0];
        int tempVal = 0;

        // Find the key that was pressed the longest
        for(int i = 1; i < keysPressed.length(); i++)
        {
            ...
            if(tempVal > val)
            {
                longestKeyPressed = keysPressed.charAt(i);
                val = tempVal;
            }
            else if(tempVal == val)
            {
                if(keysPressed.charAt(i) > longestKeyPressed)
                {
                    longestKeyPressed = keysPressed.charAt(i);
                }
            }
